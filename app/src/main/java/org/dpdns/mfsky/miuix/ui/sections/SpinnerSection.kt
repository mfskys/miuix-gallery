...
fun LazyListScope.spinnerSection() {
    item(key = "spinner") {
        ...
        SmallTitle(text = "SpinnerPreference")
        Card(
            modifier = Modifier
            ...
            OverlaySpinnerPreference(
                title = "SpinnerPref (O)",
                summary = if (overlayExpanded) "已展开" else "已收起",
                items = spinnerOptions,
                selectedIndex = superSpinnerOptionSelected.intValue,
                onSelectedIndexChange = { newOption -> superSpinnerOptionSelected.intValue = newOption },
                onExpandedChange = { overlayExpanded = it },
            )
            WindowSpinnerPreference(
                title = "SpinnerPref (W)",
                summary = if (windowExpanded) "已展开" else "已收起",
                items = spinnerOptions,
                selectedIndex = windowSpinnerOptionSelected.intValue,
                onSelectedIndexChange = { newOption -> windowSpinnerOptionSelected.intValue = newOption },
                onExpandedChange = { windowExpanded = it },
            )
            OverlaySpinnerPreference(
                title = "SpinnerPref (O · 对话框)",
                summary = "对话框形式 (O)" + if (overlayDialogExpanded) "（已展开）" else "（已收起）",
                dialogButtonString = "确定",
                items = spinnerOptions,
                selectedIndex = superSpinnerOptionSelectedDialog.intValue,
                onSelectedIndexChange = { newOption -> superSpinnerOptionSelectedDialog.intValue = newOption },
                onExpandedChange = { overlayDialogExpanded = it },
            )
            WindowSpinnerPreference(
                title = "SpinnerPref (W · 对话框)",
                summary = "对话框形式 (W)" + if (windowDialogExpanded) "（已展开）" else "（已收起）",
                dialogButtonString = "确定",
                items = spinnerOptions,
                selectedIndex = windowSpinnerOptionSelectedDialog.intValue,
                onSelectedIndexChange = { newOption -> windowSpinnerOptionSelectedDialog.intValue = newOption },
                onExpandedChange = { windowDialogExpanded = it },
            )
            OverlaySpinnerPreference(
                title = "Grouped SpinnerPref (O)",
                summary = if (overlayGroupedExpanded) "已展开" else "已收起",
            ...
            WindowSpinnerPreference(
                ...
                summary = if (windowGroupedExpanded) "已展开" else "已收起",
                entries = windowGroupedSpinnerOptions,
                collapseOnSelection = false,
                onExpandedChange = { windowGroupedExpanded = it },
            )
            OverlaySpinnerPreference(
                title = "Grouped SpinnerPref (O · 对话框)",
                summary = "对话框形式 (O)" + if (overlayGroupedDialogExpanded) "（已展开）" else "（已收起）",
                dialogButtonString = "确定",
                entries = overlayGroupedDialogSpinnerOptions,
                onExpandedChange = { overlayGroupedDialogExpanded = it },
            )
            WindowSpinnerPreference(
                title = "Grouped SpinnerPref (W · 对话框)",
                summary = "对话框形式 (W)" + if (windowGroupedDialogExpanded) "（已展开）" else "（已收起）",
                dialogButtonString = "确定",
