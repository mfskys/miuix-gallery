...
fun LazyListScope.colorPaletteSection() {
    item(key = "colorPalette") {
        ...
        var colorHex by rememberColorHex(selectedColor)
...
@Composable
private fun rememberColorHex(selectedColor: Color): MutableState<String> {
    val state = rememberSaveable {
        mutableStateOf(selectedColor.toArgb().toHexString(HexFormat.UpperCase))
    }
    LaunchedEffect(selectedColor) {
        val hex = selectedColor.toArgb().toHexString(HexFormat.UpperCase)
        if (state.value != hex) state.value = hex
    }
    return state
}
